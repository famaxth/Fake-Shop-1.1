...
def send_welcome(message):
    if not str(message.chat.id) in joinedUsers:
        ...
        bot.send_message(message.chat.id, ""+config.name+"\n\nДобро пожаловать, "+message.chat.first_name+"!\n\nВы можете совершить покупку и получить свой товар сразу после оплаты.\nВыдача адресов круглосуточно без участия оператора!\nВсе безопасно и анонимно\n\nЕсли возникнут какие-то проблемы - @admin", reply_markup=menu.operator)
...
def send_otziv(message):
    if message.text == 'Покупки':
        ...
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, "Админ панель", reply_markup=menu.admin)
        else:
            bot.send_message(message.chat.id, "❌ В доступе отказано!")
        ...
        if message.chat.id == config.admin_id:
            print('Пользователь нажал информация. ID: '+str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
            bot.send_message(message.chat.id, "Ваш текст", reply_markup=menu.start2)
        else:
            print('Пользователь нажал информация. ID: '+str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
            bot.send_message(message.chat.id, "Ваш текст", reply_markup=menu.start)
        ...
        bot.send_message(message.chat.id, 'Какой район интересует?', reply_markup=menu.rayon_msk)
    elif message.text == 'Санкт-Петербург':
        print("Выбрал г. Санкт-Петербург. ID: "+str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, 'Какой район интересует?', reply_markup=menu.rayon_spb)
    elif message.text == 'Екатеринбург':
        print("Выбрал г. Екатеринбург. ID: "+str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, 'Какой район интересует?', reply_markup=menu.rayon_ekb)
        ...
        bot.send_message(message.chat.id, 'Какой район интересует?', reply_markup=menu.rayon_perm)
    elif message.text == 'Подольск':
        print("Выбрал г. Подольск. ID: "+str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, 'Какой район интересует?', reply_markup=menu.rayon_voronezh)
